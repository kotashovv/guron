document.addEventListener('DOMContentLoaded', ()=>{
	...
	const linkWithSublist = document.querySelectorAll('.with-sublist');
	if (linkWithSublist.length != 0) {
	
		const allSublist = document.querySelectorAll('.sub-list')

		linkWithSublist.forEach(function(item) {
		...
		document.querySelector('.header').addEventListener('mouseleave', ()=>{
			allSublist.forEach(function(item){
	...
	const backBlack = document.querySelector('.back-black');

	if (backBlack) {

		function BlackOn() {
			backBlack.classList.add('active');
		}

		function BackOff() {
			backBlack.classList.remove('active');
		}
	}
			...
			item.addEventListener('click', (e)=>{
				if (!e.target.classList.contains('active')) {
					...
					document.addEventListener('click', (e)=>{
						if (!e.target.closest('.city-btn') && !e.target.closest('.city-popup')) {
							CloseCity();
						};
					})
				} else {
